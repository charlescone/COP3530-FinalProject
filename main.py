...
tk.Button(top_frame1, text='Homepage', command=lambda: raise_frame(page1), bg=BG_COLOR,
          fg=FG_COLOR).pack(side=tk.LEFT)
tk.Button(top_frame1, text='Knn Algorithm', command=lambda: raise_frame(page2), bg=BG_COLOR,
          fg=FG_COLOR).pack(side=tk.LEFT)
tk.Button(top_frame1, text='Tf-Idf Algorithm', command=lambda: raise_frame(page3), bg=BG_COLOR,
          fg=FG_COLOR).pack(side=tk.LEFT)
...
tk.Button(top_frame2, text='Knn Algorithm', command=lambda: raise_frame(page2), bg=BG_COLOR,
          fg=FG_COLOR).pack(side=tk.LEFT)
tk.Button(top_frame2, text='Go to Homepage', command=lambda: raise_frame(page1), bg=BG_COLOR,
          fg=FG_COLOR).pack(side=tk.LEFT)
tk.Button(top_frame2, text='Tf-Idf Algorithm', command=lambda: raise_frame(page3), bg=BG_COLOR,
          fg=FG_COLOR).pack(side=tk.LEFT)
...
label_KNN = tk.Label(center_frame2, text="Input a movie title in the box below\nthat has similar genre and style\nas "
                                         "the movie you would like to watch.\n", bg=BG_COLOR,
                     fg=FG_COLOR, font=LABEL_FONT)
label_KNN.grid(row=1, column=2)
button_knn = tk.Button(center_frame2, text="ENTER", command=enterpressed, width=20, height=2, bg=BUTTON_COLOR,
                       fg=BG_COLOR, font=BUTTON_FONT)
button_knn.grid(row=3, column=2)
center_frame2.rowconfigure(2, pad=20)
entryVal = StringVar()
entry = tk.Entry(center_frame2, textvariable=entryVal, font=ENTRY_FONT)
entry.grid(row=2, column=2, pady=(0, 10))
...
tk.Button(top_frame3, text='Tf-Idf Algorithm', command=lambda: raise_frame(page3),
          bg=BG_COLOR, fg=FG_COLOR).pack(side=tk.LEFT)
tk.Button(top_frame3, text='Go to Homepage', command=lambda: raise_frame(page1),
          bg=BG_COLOR, fg=FG_COLOR).pack(side=tk.LEFT)
tk.Button(top_frame3, text='Knn Algorithm', command=lambda: raise_frame(page2),
          bg=BG_COLOR, fg=FG_COLOR).pack(side=tk.LEFT)
...
button_tf = tk.Button(center_frame3, text="ENTER", command=enterpressed, width=20, height=2, bg=BUTTON_COLOR,
                      fg=BG_COLOR, font=BUTTON_FONT)
button_tf.grid(row=3, column=2)
center_frame3.rowconfigure(2, pad=20)
entryTF = StringVar()
entry2 = tk.Entry(center_frame3, textvariable=entryTF, font=ENTRY_FONT)
entry2.grid(row=2, column=2, pady=(0, 10))
